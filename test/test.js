...
describe('Pagination', function () {
    ...
    describe('setDefaultOptions', function () {
        it('should change pagination obj properties to default', function () {
            mainUtils.pagination.setDefaultOptions();
            assert.equal(mainUtils.pagination.postsPerPage, 10);
            assert.equal(mainUtils.pagination.from, 1);
            assert.equal(mainUtils.pagination.to, 1);
            assert.equal(mainUtils.pagination.skip, 0);
            assert.equal(mainUtils.pagination.currentPage, 0);
            assert.equal(mainUtils.pagination.rowCounter, 0);
            assert.equal(mainUtils.pagination.nextPage, 0);
            assert.equal(mainUtils.pagination.prevPage, 0);
            assert.equal(mainUtils.pagination.pageType, '');
            assert.equal(mainUtils.pagination.category, '');
        });
    });
});
...
describe('Categories', function () {
    ...
    describe('addCategoriesToPosts', function () {
        it('should add categories array to posts with categories,' +
            ...
            mainUtils.categories.postsCategories = _.cloneDeep(post_categories);
            mainUtils.categories.postsCategories.splice(3, mainUtils.categories.length);
